...
llvm::LogicalResult extractShapeFromSize(mlir::Value size, llvm::SmallVectorImpl<int64_t> &shape) {
  ...
  // Case 2: list built from scalar i64 values (e.g. MakeListOp of Mrt_I64Type).
  if (auto makeList = size.getDefiningOp<MakeListOp>()) {
    ...
    std::transform(makeList.getOperands().begin(), makeList.getOperands().end(), std::back_inserter(shape),
                   [](mlir::Value dimVal) {
                     // Only accept MRT scalar i64 dimensions with constant value.
                     if (mlir::isa<mrt::I64Type>(dimVal.getType())) {
...
mrt::DeviceAttr buildDevice(mlir::MLIRContext *ctx, mlir::StringRef deviceStr) {
...
mlir::LogicalResult inferTensorWithShapeDtypeDevice(mlir::MLIRContext *ctx, std::optional<mlir::Location> loc,
  ...
  auto dtypeTy = mlir::dyn_cast<mrt::DtypeType>(dtype.getType());
  if (!dtypeTy) {
    return mlir::emitOptionalError(loc, "dtype must be mrt.dtype type");
  }
  ...
  auto tensorTy = mrt::TensorType::get(ctx, shape, elementType, dev);
