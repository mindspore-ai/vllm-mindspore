...
struct TorchArithToMrtFusion : public OpConversionPattern<OpType> {
  ...
  LogicalResult matchAndRewrite(OpType op, typename OpType::Adaptor adaptor,
    ...
    // Convert operands to MRT types (mrt.i64)
    ...
    auto resultType = mrt::I64Type::get(rewriter.getContext());
    rewriter.replaceOpWithNewOp<mrt::EvalSymbolicExprOp>(op, resultType, convertedOperands, mapAttr);
    return success();
  }
};
...
void populateArithToMrtConversionPatterns(TypeConverter &converter, RewritePatternSet &patterns) {
  patterns.add<TorchArithToMrtFusion<AtenAddIntOp>>(converter, patterns.getContext());
  patterns.add<TorchArithToMrtFusion<AtenSubIntOp>>(converter, patterns.getContext());
  patterns.add<TorchArithToMrtFusion<AtenMulIntOp>>(converter, patterns.getContext());
  patterns.add<TorchArithToMrtFusion<AtenFloordivIntOp>>(converter, patterns.getContext());
  patterns.add<TorchArithToMrtFusion<AtenRemainderIntOp>>(converter, patterns.getContext());
}
