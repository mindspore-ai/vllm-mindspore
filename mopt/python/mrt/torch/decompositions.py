...
def _get_decomp_stack(scope: str) -> List[DecompositionTable]:
    """Returns the thread-local stack for the given scope, creating it if absent."""
    try:
...
def _extend_context_manager(
    ...
    """Context manager to push a derived decomposition table on the scope stack.

    Args:
        scope: The name of the decomposition scope (thread-local).
        from_current: If True, start from the current table; otherwise start empty.
        add_ops: Optional sequence of ops to add decompositions for.
        remove_ops: Optional sequence of ops to remove decompositions for.

    Yields:
        The table that is active within the context.
    """
