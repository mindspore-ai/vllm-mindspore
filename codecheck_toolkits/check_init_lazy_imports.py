...
"""Ensure we perform lazy loading in vllm_mindspore/__init__.py.
...
def _is_internal(name: str | None, *, level: int = 0) -> bool:
    ...
    if name is None:
        return False
    return name.startswith("vllm_mindspore.") or name == "vllm_mindspore"


def _fail(violations: Iterable[tuple[int, str]]) -> None:
    print("ERROR: Disallowed eager imports in vllm_mindspore/__init__.py:\n",
          file=sys.stderr)
