...
from typing import TypedDict, Any, Callable, Literal, Optional, Union
...
from vllm_mindspore.model_executor.models.qwen2_5_vl import _qwen2vl_field_config
    ...
    def construct(self, embeddings, lengths, image_shapes, h_coords,
        ...
        if total_seq == 0:
            ...
            if isinstance(lengths, list):
                lengths = ms.Tensor(lengths,
                                    dtype=ms.int64)
            if not isinstance(image_shapes, Tensor):
                image_shapes = ms.Tensor(image_shapes,
                                         dtype=ms.int64)
            ...
            target_h = mint.repeat_interleave(h_values, lengths).to(dtype=ms.float32)

            w_values = image_shapes[:, 2]
            target_w = mint.repeat_interleave(w_values, lengths).to(dtype=ms.float32)
...
class Glm4vVisionTransformer(nn.Cell):
    ...
    def __init__(
        ...
        self.downsample = ms.nn.Dense(vision_config.spatial_merge_size ** 2 * vision_config.hidden_size,
    ...
    def construct(
        ...
        x = x.view(-1, self.spatial_merge_size * self.spatial_merge_size * hidden_size)
        x = self.downsample(x)
        x = self.merger(x)
        ...
        for name, loaded_weight in weights:
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if name in params_dict:
                    ...
                    elif "downsample.weight" in name:
                        loaded_weight = loaded_weight[:]
                        loaded_weight = loaded_weight.reshape(loaded_weight.shape[0],
                                                              -1)
                        param.set_data(ms.Tensor(loaded_weight, dtype=param.dtype))
...
class Glm4vForConditionalGeneration(NativeModel, SupportsMultiModal):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.wrap_model = init_vllm_registered_model(
    ...
    def common_preprocess(self, vllm_config, prefix=""):
        self.set_modules({
            ...
            "model.language_model": self.wrap_model.model,
            "lm_head": self.wrap_model.lm_head
        })
    ...
    def _process_image_input(
            self, image_input: Glm4vImageInputs) -> tuple[Tensor, ...]:
    ...
    def _process_video_input(
            self, video_input: Glm4vVideoInputs) -> tuple[Tensor, ...]:
        grid_thw = video_input["video_grid_thw"]
        assert grid_thw.ndim == 2

        device = self.visual.device
        flat_grid_thw = mint.cat([
            mint.tensor([[1, h, w]] * t, device=device)
        ...
        if video_input["type"] == "video_embeds":
            ...
            pixel_values_videos = video_input["pixel_values_videos"].type(
                self.visual.dtype)
                # output = network(x, grid_thw, rotary_pos_emb, image_type_ids)
            rotary_pos_emb, image_type_ids = self.rotary_pos_emb(grid_thw)
    ...
    def compute_logits(
        ...
        return self.wrap_model.compute_logits(hidden_states,
                                         sampling_metadata)
