...
from vllm_mindspore.model_executor.models.glm4 import Glm4ForCausalLM
...
class Glm4vVisionAttention(nn.Cell):
    ...
    def __init__(
        ...
        self._is_310p = is_310p()
        if self._is_310p:
            import ms_custom_ops
            self.apply_rotary_pos_emb_cops = \
                ms_custom_ops.apply_rotary_pos_emb_cops

    def construct(self, x: ms.Tensor, batch_valid_length: ms.Tensor,
                  position_embeddings: tuple[ms.Tensor, ms.Tensor],
                  q_seq_lens: ms.Tensor) -> ms.Tensor:
        ...
        if self._is_310p:
            ...
            k = k.astype(ms.float32)
            # apply_rotary_pos_emb_cops is a inplace-op
            self.apply_rotary_pos_emb_cops(q, k, cos, sin, "BSND", "half")
        else:
...
class Glm4vVisionPatchEmbed(nn.Cell):
    ...
    def __init__(
        ...
        # replace nn.Conv3d with nn.Dense to accelerate.
...
class Glm4vPatchMerger(nn.Cell):
    ...
    def __init__(
        ...
        self.proj = ColumnParallelLinear(self.hidden_size,
                                         ...
                                         quant_config=quant_config,
                                         prefix=f"{prefix}.proj")
        self.post_projection_norm = mint.nn.LayerNorm(self.hidden_size)
        self.gate_up_proj = MergedColumnParallelLinear(
        ...
        self.down_proj = RowParallelLinear(context_dim,
                                           ...
                                           prefix=f"{prefix}.down_proj")
        self.act_fn = SiluAndMul()
        self.extra_activation_func = mint.nn.GELU()
...
class Glm4vVisionEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.position_embedding = mint.nn.Embedding(self.num_positions,
                                                    self.embed_dim,
                                                    dtype=self.dtype)
        #TODO: move import ms_custom_ops to the top of the file
        # after ms_custom_ops fix the issue.
        import ms_custom_ops
        self._use_custom_grid_sample = (is_310p() and hasattr(
            ms_custom_ops, "grid_sample"))
        if self._use_custom_grid_sample:
            self.custom_grid_sample = ms_custom_ops.grid_sample
    ...
    def construct(self, embeddings, lengths, image_shapes, h_coords,
        ...
        if total_seq == 0:
            adapted_pos_embed = mint.empty(0,
                                           hidden_size,
            ...
            target_h = mint.repeat_interleave(h_values,
                                              lengths).to(dtype=ms.float32)

            w_values = image_shapes[:, 2]
            target_w = mint.repeat_interleave(w_values,
                                              lengths).to(dtype=ms.float32)
            ...
            grid = (mint.stack((norm_w, norm_h),
                               dim=-1).unsqueeze(0).unsqueeze(2))

            if self._use_custom_grid_sample:
                pos_embed_2d = (pos_embed_weight.view(
                    orig_size, orig_size,
                    hidden_size).unsqueeze(0).astype(ms.float32))
                interpolated_embed_fp32 = self.custom_grid_sample(
                    pos_embed_2d,
                    grid,
                    0,  # "bilinear"
                    1,  #"border"
                ...
                adapted_pos_embed_fp32 = (
                    interpolated_embed_fp32.squeeze(0).squeeze(-2))
            else:
                pos_embed_2d = (pos_embed_weight.view(
                    orig_size, orig_size,
                    hidden_size).permute(2, 0,
                                         1).unsqueeze(0).astype(ms.float32))
                interpolated_embed_fp32 = F.grid_sample(
                    pos_embed_2d,
                    grid,
                    mode="bilinear",  #TODO: mode="bicubic" not support yet.
                    align_corners=False,
                    padding_mode="border",
                )
                adapted_pos_embed_fp32 = (
                    interpolated_embed_fp32.squeeze(0).squeeze(-1).permute(
                        1, 0))
...
class Glm4vVisionTransformer(nn.Cell):
    ...
    def __init__(
        ...
        # replace nn.Conv2d with nn.Dense to accelerate.
        self.downsample = ms.nn.Dense(vision_config.spatial_merge_size**2 *
    ...
    def construct(self, x: ms.Tensor, grid_thw: ms.Tensor, rotary_pos_emb,
                  image_type_ids, batch_valid_length, q_seq_lens) -> ms.Tensor:
        # patchify
        ...
        x = self.embeddings(x, batch_valid_length, grid_thw,
                            image_type_ids[:, 0], image_type_ids[:, 1])

        # transformers
        for blk in self.blocks:
            x = blk(x, batch_valid_length, position_embeddings, q_seq_lens)
        ...
        # nn.Conv2d is replaced by nn.Dense, so needs to reshape the tensor
        # before calling downsample.
        x = x.view(
            -1,
            self.spatial_merge_size * self.spatial_merge_size * hidden_size)
        x = self.downsample(x)
        x = self.merger(x)
    ...
    def load_weights(self, weights: Iterable[tuple[str, ms.Tensor]],
        ...
        for name, loaded_weight in weights:
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if name in params_dict:
                    param = params_dict[name]
                    if "patch_embed.proj.weight" in name or \
                        "downsample.weight" in name:
                        loaded_weight = loaded_weight[:]
                        loaded_weight = loaded_weight.reshape(
                            loaded_weight.shape[0], -1)
                        param.set_data(
                            ms.Tensor(loaded_weight, dtype=param.dtype))
                    else:
    ...
    # set model inputs of Vision Transformers to support dynamic shape in graph.
    def set_model_inputs(self):
        ...
        self.set_inputs(dyn_x, dyn_grid_thw, dyn_rotary_pos_emb, dyn_pos_ids,
...
class Glm4vForConditionalGeneration(NativeModel, SupportsMultiModal,
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if config.model_type == "glm4v":
            ...
            #TODO: To make 'init_vllm_registered_model' work with
            # auto-backend-selection in the future, and replace directly
            # call 'Glm4ForCausalLM' with 'init_vllm_registered_model'.
            # Now if not setting 'VLLM_MS_MODEL_BACKEND=Native',
            # the language_model will be MindFormers backend,
            # which is not supported multimodal yet.
            self.language_model = Glm4ForCausalLM(
                vllm_config=vllm_config.with_hf_config(
                    hf_config=config.text_config, architectures=architectures),
                prefix=maybe_prefix(prefix, "language_model"))
        elif config.model_type == "glm4v_moe":
            raise NotImplementedError("GLM4V MoE is not supported yet.")
        else:
            raise ValueError(
                "hf config model_type must be glm4v or glm4v_moe.")

        self.model = self.language_model.model

        # compile vocab embedding of language model
        ...
        self.rotary_pos_emb_total = Glm4vVisionRotaryEmbedding(head_dim // 2)
    ...
    def _process_image_input(
            self, image_input: Glm4vImageInputs) -> tuple[ms.Tensor, ...]:
        ...
        if image_input["type"] == "image_embeds":
            ...
            rotary_pos_emb, pos_ids = self.rot_pos_emb(grid_thw)
            ...
            image_embeds = self.visual(pixel_values, grid_thw, rotary_pos_emb,
                                       pos_ids, batch_valid_length,
                                       batch_valid_length)
    ...
    def _process_video_input(
            self, video_input: Glm4vVideoInputs) -> tuple[ms.Tensor, ...]:
        ...
        if video_input["type"] == "video_embeds":
            ...
            batch_valid_length = mint.repeat_interleave(
                grid_thw_1 * grid_thw_2, grid_thw_0).astype(ms.int32)
            video_embeds = self.visual(pixel_values_videos, grid_thw,
