...
from research.deepseek3.deepseek3_model_infer import (
    InferenceDeepseekV3ForCausalLM)
from research.deepseek3.deepseekv3_infer_parallelism import (
    DeepseekInferParallelism)
...
# --load_checkpoint /path/to/safetensor_path "
# 4 8555 "output/deepseek_msrun_log" "False" 7200


def create_ptq():
    '''create_ptq'''
    from mindspore_gs.common import BackendTarget
    from mindspore_gs.ptq import (PTQ, OutliersSuppressionType,
    ...
    ffn_config = PTQConfig(
        ...
        weight_quant_granularity=QuantGranularity.PER_CHANNEL)
    ptq = PTQ(config=cfg,
              layer_policies=OrderedDict({r'.*\.feed_forward\..*':
                                          ffn_config}))
...
def main(config_path, load_checkpoint, save_ckpt_dir):
    ...
    save_ckpt_path = os.path.join(save_ckpt_dir, "rank_" + rank_id,
                                  "checkpoint_" + rank_id + ".ckpt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path',
                        default='predict_llama2_7b.yaml',
                        type=str,
                        help='model config file path.')
    parser.add_argument('--load_checkpoint',
                        type=str,
                        help='load model checkpoint path or directory.')
    parser.add_argument('--save_ckpt_dir', type=str, help='save ckpt path.')
