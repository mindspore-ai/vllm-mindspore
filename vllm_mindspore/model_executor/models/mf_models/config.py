...
MODEL_COMMON_MAPPING = {
    ...
    'max_position_embeddings': ('model_config.hf_config.max_position_embeddings', None),
    'num_attention_heads': ('model_config.hf_config.num_attention_heads', None),
    ...
    'num_key_value_heads': ('model_config.hf_config.num_key_value_heads', None),
...
def transform_config(mapping_table: dict, vllm_config: VllmConfig, target_config):
    for target_path, mapping in mapping_table.items():
        src_path, transform = mapping
        
        src_value = get_nested_attr(vllm_config, src_path) if src_path is not None else None
        
        if src_value is not None:
            transformed_value = src_value
        elif transform and isinstance(transform, (types.FunctionType, types.BuiltinFunctionType)):
...
def gen_model_config(mf_config: MindFormerConfig, model_config_type: PretrainedConfig): 
