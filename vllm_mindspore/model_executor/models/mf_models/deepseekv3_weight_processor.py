...
from vllm_mindspore.model_executor.model_loader.utils import (
   convert_uint64_to_fp32, unpack_int8_to_int4_3d, np_int4data_pack_to_int8_3d)
...
class DeepseekV3WeightProcessor(BaseWeightProcessor):
    ...
    def __init__(self, config, network, is_quant, vllm_config):
        ...
        self.col_moe_split_axis = 2 if self.is_310p else 1
        self.row_moe_split_axis = 1 if self.is_310p else 2
        ...
        logger.debug(
            ...
            "tp_rank_id: %d \n"
            "ep_method: %s \n"
            "num_router_experts: %d \n"
            "ep_group_nums: %d \n"
            "moe_ep_rank_id: %d \n"
            "moe_tp_rank_id: %d \n"
            "moe_ep_size: %d \n"
            "moe_tp_size: %d", self.global_rank_id, self.tp_group_size,
    ...
    def small_cache_weight(self, src_hf_dir, layer_id, hf_weight_map,
                           parameter_dict):
        k_nope_weight_name = (f"model.layers.{layer_id}.attention."
                              f"lkv2kv_k_nope.weight")
        k_nope_weight_param, _ = self.get_safetensor_from_file_split_tp_group(
            k_nope_weight_name, src_hf_dir,hf_weight_map, split_axis=0)
        qabsorb_matmul_name = (f"model.layers.{layer_id}.attention."
                               f"qabsorb_matmul.weight")
        qabsorb_param = k_nope_weight_param.reshape(-1, 128, 512)
        parameter_dict[qabsorb_matmul_name] = ms.Parameter(
            ms.Tensor(qabsorb_param, dtype=ms.float16),
            name=qabsorb_matmul_name, requires_grad=False)

        v_weight_name = f"model.layers.{layer_id}.attention.lkv2kv_v.weight"
        v_weight_param, _ = self.get_safetensor_from_file_split_tp_group(
            v_weight_name, src_hf_dir, hf_weight_map, split_axis=0)
        outabsorb_matmul_name = (f"model.layers.{layer_id}.attention."
                                 f"outabsorb_matmul.weight")
        outabsorb_param = v_weight_param.reshape(-1, 128, 512)
        parameter_dict[outabsorb_matmul_name] = ms.Parameter(
            ms.Tensor(outabsorb_param, dtype=ms.float16),
            name=outabsorb_matmul_name, requires_grad=False)
    ...
    def infer_quant_process_moe_with_ep(self, src_hf_dir, hf_weight_map,
        ...
        for index in range(self.ep_start, self.ep_stop):
            base_path = f"model.layers.{layer_id}.mlp.experts.{index}"
            w1_hf_name = f"{base_path}.gate_proj.weight"
            w2_hf_name = f"{base_path}.down_proj.weight"
            w3_hf_name = f"{base_path}.up_proj.weight"
            ...
            w1_scale_hf_name = f"{base_path}.gate_proj.weight_scale"
            w2_scale_hf_name = f"{base_path}.down_proj.weight_scale"
            w3_scale_hf_name = f"{base_path}.up_proj.weight_scale"
        ...
        return (w1_list, w2_list, w3_list, w1_scale_list, w2_scale_list,
                w3_scale_list)
    ...
    def infer_quant_process_moe_with_ep_tp(self, src_hf_dir, hf_weight_map,
        ...
        for index in range(self.ep_start, self.ep_stop):
            ...
            w1_scale_ms_param, _ = (
                self.get_safetensor_from_file_split_moe_tp_group(
    ...
    def infer_quant_process_moe_routed_expert_ffn_weight(
        ...
        base_path = f"model.layers.{layer_id}.feed_forward.routed_experts.ffn"
        w1_ms_name = f"{base_path}.w1._layer.weight"
        w2_ms_name = f"{base_path}.w2._layer.weight"
        w3_ms_name = f"{base_path}.w3._layer.weight"

        w1_scale_ms_name = f"{base_path}.w1._layer.matmul.weight_scale"
    ...
    def get_quant_moe_shared_expert_weight(self, w1_hf_name, w2_hf_name,
        ...
        return (w1_ms_param, w2_ms_param, w3_ms_param, w1_scale_ms_param,
                w2_scale_ms_param, w3_scale_ms_param)

    def infer_quant_process_moe_shared_expert_ffn_weight(
            self, src_hf_dir, layer_id, hf_weight_map):
        """infer quant process moe shared expert ffn weight"""
        ffn_concat = self.config.model.model_config.ffn_concat
        base_path = f"model.layers.{layer_id}.mlp.shared_experts"
        w1_hf_name = f"{base_path}.gate_proj.weight"
        w2_hf_name = f"{base_path}.down_proj.weight"
        w3_hf_name = f"{base_path}.up_proj.weight"
        ...
        (w1_ms_param, w2_ms_param, w3_ms_param, w1_scale_ms_param,
         w2_scale_ms_param,
         w3_scale_ms_param) = self.get_quant_moe_shared_expert_weight(
             w1_hf_name, w2_hf_name, w3_hf_name, w1_scale_hf_name,
             w2_scale_hf_name, w3_scale_hf_name, src_hf_dir, hf_weight_map)
        ...
        if ffn_concat:
            ...
            w_scale_gate_hidden_name = \
                f"{base_path2}.w_gate_hidden._layer.matmul.weight_scale"
            w_scale_gate_hidden_np = np.concatenate(
    ...
    def quant_special_attention_weight(self,
        ...
        input_scale_hf_name = base_name + name + ".input_scale"
        ...
        if not is_trans_rope_weigh:
            ...
            dequant_scale_hf_name = base_name + name + ".deq_scale"
    ...
    def infer_quant_process_attention_weight(self, src_hf_dir, layer_id,
        ...
        base_name = f"model.layers.{layer_id}.self_attn"
        # q_a_layernorm->lq_norm
        lq_norm_hf_name = f"{base_name}.q_a_layernorm.weight"
        ...
        # q_b_proj->l2q_proj
        l2q_proj_hf_name = f"{base_name}.q_b_proj.weight"
        ...
        lkv_norm_hf_name = f"{base_name}.kv_a_layernorm.weight"
        ...
        # kv_b_proj->lkv2kv
        lkv2kv_hf_name = f"{base_name}.kv_b_proj.weight"
        ...
        value_k_nope, value_v = (lkv2kv_ms_param[:, :qk_nope_head_dim, :],
                                 lkv2kv_ms_param[:, qk_nope_head_dim:, :])
    ...
    def convert_mtp_weight_name(self, weight_name: str):
        ...
        is_mtp_layer = ('tok_embeddings' not in weight_name
                        and 'shared_head.' not in weight_name)
        mtp_prefix = mtp_prefix if not is_mtp_layer else f'{mtp_prefix}.layer'
        is_decode_layer = ("ffn" in weight_name or "attention" in weight_name
                           or "feed_forward" in weight_name)
        mtp_prefix = (mtp_prefix
                      if not is_decode_layer else f'{mtp_prefix}.decode_layer')
    ...
    def infer_process_moe_with_ep_tp(self, src_hf_dir, hf_weight_map,
        ...
        for index in range(self.ep_start, self.ep_stop):
            w1_hf_name = f"model.layers.{layer_id}.mlp.experts.{index}." \
                         f"gate_proj.weight"
            w2_hf_name = f"model.layers.{layer_id}.mlp.experts.{index}." \
                         f"down_proj.weight"
            w3_hf_name = f"model.layers.{layer_id}.mlp.experts.{index}." \
                         f"up_proj.weight"
    ...
    def infer_process_moe_shared_expert_ffn_weight(self, src_hf_dir, layer_id,
        ...
        ffn_concat = self.config.model.model_config.ffn_concat
        base_name = f"model.layers.{layer_id}.mlp.shared_experts"
        w1_hf_name = f"{base_name}.gate_proj.weight"
        w2_hf_name = f"{base_name}.down_proj.weight"
        w3_hf_name = f"{base_name}.up_proj.weight"
        ...
        (w1_ms_param, w2_ms_param,
         w3_ms_param) = self.get_moe_shared_expert_weight(
    ...
    def infer_process_dense_ffn_weight(self, src_hf_dir, layer_id,
        ...
        if ffn_concat:
            w_gate_hidden_name = (
                f"model.layers.{layer_id}.feed_forward.w_gate_hidden.weight")
            w_gate_hidden_np = np.concatenate([w1_ms_param, w3_ms_param],
                                              axis=0)
            w_gate_hidden_param = ms.from_numpy(w_gate_hidden_np).astype(
                ms.bfloat16)
    ...
    def infer_process_attention_weight(self, src_hf_dir, layer_id,
        ...
        if qkv_concat:
            wqkv2l_weight = np.concatenate((q_a_proj_ms_param, kv2l_ms_param),
                                           0)
            wqkv2l_weight_name = (
                f"model.layers.{layer_id}.attention.qkv2l.weight")
    ...
    def infer_process_norm_weight(self, src_hf_dir, layer_id, hf_weight_map):
        ...
        # attention_norm
        attention_norm_hf_name = f"{layer_name}.input_layernorm.weight"
        ...
        # ffn_norm
        ffn_norm_hf_name = f"{layer_name}.post_attention_layernorm.weight"
    ...
    def infer_process_mtp_layer_weight(self, src_hf_dir, layer_id,
        ...
        for prefix_name in mtp_layer_names:
            ...
            if (prefix_name in head_names
                    and not self.config.parallel_config.vocab_emb_dp):
    ...
    def get_smooth_quant_moe_shared_expert_weight(
        ...
        if self.ep_method in [EPMethod.DEFAULT, EPMethod.ALLGATHER]:
            ...
            w1_scale_param, _ = (
                self.get_safetensor_from_file_split_moe_tp_group(w1_scale_name,
                                                                 src_hf_dir,
                                                                 hf_weight_map,
                                                                 split_axis=0))
            ...
            w3_scale_param, _ = (
                self.get_safetensor_from_file_split_moe_tp_group(w3_scale_name,
                                                                 src_hf_dir,
                                                                 hf_weight_map,
                                                                 split_axis=0))
    ...
    def smooth_quant_process_shared_ffn_weight(self, src_hf_dir, layer_id,
        ...
        (w1_weight_param, w1_scale_param, w3_weight_param, w3_scale_param,
         w2_weight_param) = self.get_smooth_quant_moe_shared_expert_weight(
        ...
        w_scale_dtype = ms.float32 if self.is_310p else ms.bfloat16
        ...
        if ffn_concat:
            ...
            parameter_dict[w1_scale_name] = ms.Parameter(ms.Tensor(
                w1_scale_param, w_scale_dtype),
                                                         name=w1_scale_name,
                                                         requires_grad=False)
            parameter_dict[w3_scale_name] = ms.Parameter(ms.Tensor(
                w3_scale_param, w_scale_dtype),
                                                         name=w3_scale_name,
                                                         requires_grad=False)
    ...
    def smooth_quant_process_qkv_weight(self, src_hf_dir, layer_id,
        ...
        qkv_concat = self.config.model.model_config.qkv_concat
        # q2l_proj
        base_name = f"model.layers.{layer_id}.attention"
        q2l_weight_name = f"{base_name}.q2l_proj._layer.weight"
        q2l_weight_param, _ = self.get_safetensor_from_file(
            q2l_weight_name, src_hf_dir, hf_weight_map)
        q2l_bias_name = f"{base_name}.q2l_proj._layer.matmul.quant_bias"
        q2l_bias_param, _ = self.get_safetensor_from_file(
            q2l_bias_name, src_hf_dir, hf_weight_map)
        q2l_scale_name = f"{base_name}.q2l_proj._layer.matmul.dequant_scale"
        q2l_scale_param, _ = self.get_safetensor_from_file(
            q2l_scale_name, src_hf_dir, hf_weight_map)

        q2l_quant_zp = f"{base_name}.q2l_proj.quant_op.input_zp"
        q2l_quant_scale = f"{base_name}.q2l_proj.quant_op.input_scale"
        ...
        kv2l_weight_name = f"{base_name}.kv2l._layer.weight"
        kv2l_weight_param, _ = self.get_safetensor_from_file(
            kv2l_weight_name, src_hf_dir, hf_weight_map)
        kv2l_bias_name = f"{base_name}.kv2l._layer.matmul.quant_bias"
        kv2l_bias_param, _ = self.get_safetensor_from_file(
            kv2l_bias_name, src_hf_dir, hf_weight_map)
        kv2l_scale_name = f"{base_name}.kv2l._layer.matmul.dequant_scale"
        kv2l_scale_param, _ = self.get_safetensor_from_file(
            kv2l_scale_name, src_hf_dir, hf_weight_map)

        kv2l_quant_zp = f"{base_name}.kv2l.quant_op.input_zp"
        kv2l_quant_scale = f"{base_name}.kv2l.quant_op.input_scale"
        ...
        if self.is_310p:
            q2l_scale_param = q2l_scale_param.astype(np.float32).view(np.int32).astype(np.int64)
            kv2l_scale_param = kv2l_scale_param.astype(np.float32).view(np.int32).astype(np.int64)
            deq_scale_dtype = ms.int64
            quant_scale_dtype = ms.float16
        else:
            deq_scale_dtype = ms.float32
        ...
        if qkv_concat:
            qkv2l_weight_name = f"{base_name}.qkv2l._layer.weight"
            qkv2l_bias_name = f"{base_name}.qkv2l._layer.matmul.quant_bias"
            qkv2l_scale_name = f"{base_name}.qkv2l._layer.matmul.dequant_scale"
            qkv2l_quant_zp_name = f"{base_name}.qkv2l.quant_op.input_zp"
            qkv2l_quant_scale_name = f"{base_name}.qkv2l.quant_op.input_scale"
            ...
            parameter_dict[qkv2l_quant_zp_name] = ms.Parameter(
                ...
                requires_grad=False)
            parameter_dict[qkv2l_quant_scale_name] = ms.Parameter(
                ms.Tensor(q2l_quant_scale_param, quant_scale_dtype),
            ...
            parameter_dict[q2l_scale_name] = ms.Parameter(ms.Tensor(
                q2l_scale_param, deq_scale_dtype),
                                                          name=q2l_scale_name,
                                                          requires_grad=False)
            parameter_dict[kv2l_scale_name] = ms.Parameter(
                ms.Tensor(kv2l_scale_param, deq_scale_dtype),
            ...
            parameter_dict[kv2l_quant_scale] = ms.Parameter(
                ms.Tensor(kv2l_quant_scale_param, quant_scale_dtype),
    ...
    def infer_gptq_quant_net_ms_convert_layer_weight(self, src_hf_dir,
        ...
        for param_name, _ in tqdm(hf_weight_map.items(),
            ...
            if any([name in param_name for name in no_need_split_layer]):
                ...
                if not self.config.parallel_config.vocab_emb_dp:
                    value, is_int4 = (
                        self.get_safetensor_from_file_split_tp_group(
                            param_name,
                            src_hf_dir,
                            hf_weight_map,
                            split_axis=0))
                ...
                raise ValueError(f"not found layer {param_name}, "
                                 "please check safetensors file.")
    ...
    def process_route_ffn_weight_a8w4(self, src_hf_dir, layer_id,
        ...
        ffn_concat = self.config.model.model_config.ffn_concat
        w1_weight_name = (f"model.layers.{layer_id}.{layer_type}.w1."
                          f"_layer.weight")
        w1_scale_name = (f"model.layers.{layer_id}.{layer_type}.w1."
                         f"_layer.matmul.weight_scale")
        w1_bias_name = (f"model.layers.{layer_id}.{layer_type}.w1."
                        f"_layer.matmul.gmm_bias")

        w3_weight_name = (f"model.layers.{layer_id}.{layer_type}.w3."
                          f"_layer.weight")
        w3_scale_name = (f"model.layers.{layer_id}.{layer_type}.w3."
                         f"_layer.matmul.weight_scale")
        w3_bias_name = (f"model.layers.{layer_id}.{layer_type}.w3."
                        f"_layer.matmul.gmm_bias")

        w2_weight_name = (f"model.layers.{layer_id}.{layer_type}.w2."
                          f"_layer.weight")
        w2_scale_name = (f"model.layers.{layer_id}.{layer_type}.w2."
                         f"_layer.matmul.weight_scale")
        w2_bias_name = (f"model.layers.{layer_id}.{layer_type}.w2."
                        f"_layer.matmul.gmm_bias")

        w1_weight_param, _ = self.get_routed_safetensor_3_dim(
            w1_weight_name, src_hf_dir, hf_weight_map, split_ep=False,
            split_tp=True, tp_axis=self.col_moe_split_axis)
        w1_scale_param, _ = self.get_routed_safetensor_3_dim(w1_scale_name,
                                                             src_hf_dir,
                                                             hf_weight_map,
                                                             split_ep=False,
                                                             split_tp=True,
        ...
        w3_weight_param, _ = self.get_routed_safetensor_3_dim(
            ...
            split_tp=True, tp_axis=self.col_moe_split_axis)
        w3_scale_param, _ = self.get_routed_safetensor_3_dim(w3_scale_name,
                                                             src_hf_dir,
                                                             hf_weight_map,
                                                             split_ep=False,
                                                             split_tp=True,
        ...
        w2_weight_param, _ = self.get_routed_safetensor_3_dim(
            ...
            split_tp=True, tp_axis=self.row_moe_split_axis)
        ...
        if self.is_310p:
            ...
            w1_weight_param = unpack_int8_to_int4_3d(w1_weight_param)
            w1_weight_param = np_int4data_pack_to_int8_3d(
                w1_weight_param.transpose(0, 2, 1))

            w2_weight_param = w2_weight_param.astype(np.int8)
            w2_weight_param = unpack_int8_to_int4_3d(w2_weight_param)
            w2_weight_param = np_int4data_pack_to_int8_3d(
                w2_weight_param.transpose(0, 2, 1))

            w3_weight_param = w3_weight_param.astype(np.int8)
            w3_weight_param = unpack_int8_to_int4_3d(w3_weight_param)
            w3_weight_param = np_int4data_pack_to_int8_3d(
                w3_weight_param.transpose(0, 2, 1))

            w1_scale_param = convert_uint64_to_fp32(w1_scale_param)
            w2_scale_param = convert_uint64_to_fp32(w2_scale_param)
            w3_scale_param = convert_uint64_to_fp32(w3_scale_param)
            w_scale_dtype = ms.float32
        else:
            w_scale_dtype = ms.uint64


        if ffn_concat:
            concat_weight_name = (f"model.layers.{layer_id}.{layer_type}."
                                  f"w_gate_hidden._layer.weight")
            concat_weight_param = ms.Tensor(np.concatenate(
                [w1_weight_param, w3_weight_param], axis=1), dtype=ms.qint4x2)
            parameter_dict[concat_weight_name] = ms.Parameter(
                concat_weight_param, name=concat_weight_name,
                requires_grad=False)

            concat_scale_name = (f"model.layers.{layer_id}.{layer_type}."
                                 f"w_gate_hidden._layer.matmul.weight_scale")
            concat_scale_param = ms.Tensor(np.concatenate(
                [w1_scale_param, w3_scale_param], axis=2),
                dtype=w_scale_dtype)
            parameter_dict[concat_scale_name] = ms.Parameter(
                concat_scale_param, name=concat_scale_name,
                requires_grad=False)

            concat_scale_name = (f"model.layers.{layer_id}.{layer_type}."
                                 f"w_gate_hidden._layer.matmul.gmm_bias")
            concat_scale_param = ms.Tensor(np.concatenate(
                [w1_bias_param, w3_bias_param], axis=1), dtype=ms.float32)
            parameter_dict[concat_scale_name] = ms.Parameter(
                concat_scale_param, name=concat_scale_name,
                requires_grad=False)
        else:
            # w1 w3
            parameter_dict[w1_weight_name] = ms.Parameter(ms.Tensor(
                w1_weight_param, ms.qint4x2), name=w1_weight_name,
                requires_grad=False)
            parameter_dict[w3_weight_name] = ms.Parameter(ms.Tensor(
                w3_weight_param, ms.qint4x2), name=w3_weight_name,
                requires_grad=False)
        ...
        parameter_dict[w2_weight_name] = ms.Parameter(ms.Tensor(
            w2_weight_param, ms.qint4x2), name=w2_weight_name,
            requires_grad=False)
        parameter_dict[w2_scale_name] = ms.Parameter(ms.Tensor(
            w2_scale_param, w_scale_dtype),
        ...
        parameter_dict[w2_bias_name] = ms.Parameter(ms.Tensor(
            ...
            name=w2_bias_name, requires_grad=False)


    def infer_a8w4_get_value(self, param_name, src_hf_dir,
                             hf_weight_map, no_need_split_layer):
        '''infer_smooth_quant_get_value'''

        if any([name in param_name for name in no_need_split_layer]):
            value, is_int4 = self.get_safetensor_from_file(param_name,
                                                           src_hf_dir,
                                                           hf_weight_map)
        elif any([name in param_name for name in [".l2q_proj."]]):
            if param_name.endswith(".weight") or "matmul" in param_name:
                value, is_int4 = self.get_safetensor_from_file_split_tp_group(
                    param_name, src_hf_dir, hf_weight_map, split_axis=0)
                if self.is_310p and "dequant_scale" in param_name:
                    value = value.view(np.int32).astype(np.int64)
            else:
                value, is_int4 = self.get_safetensor_from_file(param_name,
                                                               src_hf_dir,
                                                               hf_weight_map)
                                                  ...
                                                  "shared_experts.w2"]]):
            value = self.infer_smooth_quant_row_linear_split(param_name,
                                                             src_hf_dir,
                                                             hf_weight_map)
            if self.is_310p and "dequant_scale" in param_name:
                value = value.view(np.int32).astype(np.int64)
            ...
            raise ValueError(f"not found layer {param_name},"
                             f" please check safetensors file.")
        return value, is_int4

    def infer_a8w4_net_ms_convert_layer_weight(self, src_hf_dir,
                                               hf_weight_map):
        '''infer_qqq_net_ms_convert_layer_weight'''
        parameter_dict = {}
        network_names = []
        for m in self.network.parameters_and_names():
            network_names.append(m[0])
        no_need_split_layer = ["tok_embeddings", "norm",
                               "routed_experts.router.dense",
                               "routed_experts.router.e_score_correction_bias",
                               "topk_bias"]
        for layer_id in tqdm(range(self.start_layer, self.end_layer),
                             desc="qkv/ffn params load"):
            if layer_id >= 3:
            ...
            if self.is_310p:
                self.small_cache_weight(src_hf_dir, layer_id, hf_weight_map,
                                        parameter_dict)

        skip_layer = ["feed_forward.routed_experts.ffn",
                      "feed_forward.shared_experts", "feed_forward.w",
                      "attention.kv2l", "attention.q2l", "attention.qkv"]

        for param_name, _ in tqdm(hf_weight_map.items(),
                                  desc="remaining params load"):
            if param_name not in network_names:
                continue

            if any([name in param_name for name in skip_layer]):
                continue

            value, is_int4 = self.infer_a8w4_get_value(param_name,
                                                       src_hf_dir,
                                                       hf_weight_map,
                                                       no_need_split_layer)
            dst_dtype = convert_np_to_ms_dtype(value)
    ...
    def load_safetensors_shard(self, src_hf_dir, is_mtp_model=False):
        ...
        for file in os.listdir(src_hf_dir):
            if file.endswith('index.json'):
                # mtp model do not support quantization,
                # needs to load bf16 weight.
                if ('quant' in file and self.is_quant) or \
                        ('quant' not in file and \
                            (not self.is_quant or is_mtp_model)):
