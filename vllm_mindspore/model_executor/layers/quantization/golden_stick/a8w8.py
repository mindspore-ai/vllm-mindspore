...
class A8W8LinearMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: QuantizationConfig) -> None:
        ...
        self.is_ms_custom_ops = ms_custom_ops

    def create_weights(self, layer: nn.Cell, input_size_per_partition: int,
                       output_partition_sizes: list[int], params_dtype,
                       **extra_weight_attrs):
        output_size = sum(output_partition_sizes)
        self.output_size = output_size
        self.input_size_per_partition = input_size_per_partition
        self.params_dtype = params_dtype
        self.matmul = ops.auto_generate.QuantBatchMatmul(transpose_x2=True,
    ...
    def process_weights_after_loading(self, layer: nn.Cell) -> None:
        if self.is_ms_custom_ops:
            layer.weight = ms_custom_ops.trans_data(layer.weight,
                                                    transdata_type=1)
        if not self.is_modelslim:
            return
        input_scale = 1 / layer.input_scale.numpy()
    ...
    def apply(self,
        ...
        if self.is_ms_custom_ops:
            out = ms_custom_ops.quant_batch_matmul(
                qx,
                weight,
                deq_scale,
                None,
