...
from mindspore.ops.auto_generate import QuantBatchMatmul, DynamicQuantExt, GroupedMatmulV4, GroupedMatmul
...
class SmoothQuantConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: mindspore.nn.Cell,
        ...
        if is_layer_skipped_a8w8(prefix, self.modules_to_not_convert):
            return UnquantizedLinearMethod()
        quant_key = build_layer_quant_key(prefix)

        if isinstance(layer, Attention):
            if layer.cache_config.cache_dtype == "int8" and self.full_config.get(quant_key) == 'C8':
                return KVCacheInt8Method(self)
            return BaseKVCacheMethod(self)

        if isinstance(layer, LinearBase):
            if self.full_config.get(quant_key) == 'W8A8':
                return A8W8LinearMethod(self)
            if self.full_config.get(quant_key) == 'W8A8_DYN':
                self.dynamic_quant = True
                return A8W8DYNLinearMethod(self)

        print(f'get_quant_method unmatched {layer.__class__.__name__, quant_key, self.full_config.get(quant_key)}')
        return UnquantizedLinearMethod()

def build_layer_quant_key(prefix: str) -> str:
...
def is_layer_skipped_a8w8(prefix: str, modules_to_not_convert: list[str]):
...
class A8W8LinearMethod(LinearMethodBase):
    ...
    def create_weights(self,
        ...
        self.gmm_transpose = self.atlas_inference
        if self.is_group_mm:
            if self.atlas_inference:
                self.matmul = GroupedMatmul(split_item=3, group_type=0, transpose_a=False, transpose_b=True)
            else:
                self.matmul = GroupedMatmulV4()
            expert_num_per_partition = expert_num_per_partition
            shape = (expert_num_per_partition, input_size_per_partition, output_size_per_partition // self.quant_config.pack_factor)
            if self.gmm_transpose:
                shape = (shape[0], shape[2], shape[1])
            weight = Parameter(initializer('ones', shape, mindspore.int8))
            shape = (expert_num_per_partition, output_size_per_partition // self.quant_config.pack_factor)
            scale_dtype = mindspore.bfloat16 if params_dtype == mindspore.bfloat16 else mindspore.float32
            weight_scale = Parameter(initializer('ones', shape, scale_dtype), name="weight_scale")
            scale_dtype = mindspore.int64
            deq_scale = Parameter(initializer('ones', shape, scale_dtype), name="deq_scale")
            shape = (expert_num_per_partition,)
            input_scale = Parameter(initializer('ones', shape, params_dtype), name="input_scale")
            input_offset = Parameter(initializer('zeros', shape, params_dtype),  name="input_offset")
            quant_bias_ = None
            ...
            shape = (output_size_per_partition // self.quant_config.pack_factor, input_size_per_partition)
            weight = Parameter(initializer('ones', shape, mindspore.int8))
            shape = (output_size_per_partition // self.quant_config.pack_factor, 1)
            scale_dtype = mindspore.bfloat16 if params_dtype == mindspore.bfloat16 else mindspore.float32
            weight_scale = Parameter(initializer('ones', shape, scale_dtype), name="weight_scale")
            shape = (output_size_per_partition // self.quant_config.pack_factor)
            scale_dtype = mindspore.int64
            deq_scale = Parameter(initializer('ones', shape, scale_dtype), name="deq_scale")
            shape = (1,)
            input_scale = Parameter(initializer('ones', shape, params_dtype), name="input_scale")
            input_offset = Parameter(initializer('zeros', shape, params_dtype),  name="input_offset")
            if self.atlas_inference:
                quant_bias_ = Parameter(initializer('zeros',
                                                    (output_size_per_partition // self.quant_config.pack_factor, ),
                                                    mindspore.int32),  name="quant_bias_")
            else:
                quant_bias_ = None
    ...
    def process_weights_after_loading(self, layer: mindspore.nn.Cell) -> None:
        ...
        if self.is_group_mm:
            ...
            layer.input_scale = Parameter(Tensor(input_scale, dtype = layer.input_scale.dtype), name=layer.input_scale.name)
            if self.atlas_inference:
                layer.weight_scale = Parameter(Tensor(weight_scale.view(np.int32).astype(np.int64),
                                                      dtype=mindspore.int64), name=layer.weight_scale.name)
            else:
                layer.weight_scale = Parameter(Tensor(weight_scale, dtype=layer.weight_scale.dtype),
                                               name=layer.weight_scale.name)
        if not self.atlas_inference and params_dtype is mindspore.bfloat16:
            deq_scale = layer.deq_scale.asnumpy().astype(np.int32).view(np.float32)
            layer.deq_scale = Parameter(Tensor(deq_scale, dtype = mindspore.float32), name=layer.deq_scale.name)
    ...
    def apply(self,
        ...
        if self.is_group_mm:
            if self.atlas_inference:
                ...
                qx = self.matmul([qx], [weight], None, [weight_scale], None,
                         None, None, None, group_list, split_item=3, group_type=0, group_list_type=0 if cumsum_flag else 1)[0]
        else:
            qx = self.matmul(qx, weight, deq_scale, None, layer.quant_bias_, None)
        if bias is not None:
...
class A8W8DYNLinearMethod(LinearMethodBase):
    ...
    def create_weights(self,
        ...
        if self.is_group_mm:
            ...
            weight = Parameter(initializer('ones', shape, mindspore.int8))
            shape = (expert_num_per_partition, output_size_per_partition // self.quant_config.pack_factor)
            weight_scale = Parameter(initializer('ones', shape, params_dtype))

            shape = (input_size_per_partition,)
            smooth_scale = Parameter(initializer('ones', shape, params_dtype))
            set_weight_attrs(smooth_scale, {"input_dim": 0})

            set_weight_attrs(weight, {"ep_dim":0, "input_dim": 1, "output_dim": 2})
            set_weight_attrs(weight_scale, {"ep_dim":0, "output_dim": 1})
        else:
            self.matmul = QuantBatchMatmul(transpose_x1=False, transpose_x2=True, dtype=mindspore.bfloat16)
            shape = (output_size_per_partition // self.quant_config.pack_factor, input_size_per_partition)
            weight = Parameter(initializer('ones', shape, mindspore.int8))
            shape = (output_size_per_partition // self.quant_config.pack_factor)
            weight_scale = Parameter(initializer('ones', shape, params_dtype))
    ...
    def process_weights_after_loading(self, layer: mindspore.nn.Cell) -> None:
        if self.is_2d_smooth_scale:
            ...
            layer.smooth_scale = Parameter(Tensor(smooth_scale, dtype=mindspore.float32), name=layer.smooth_scale.name)
    ...
    def apply(self,
        ...
        qx = qx.reshape(-1, self.input_size_per_partition)
        if self.is_group_mm:
            qx = self.matmul([qx], [weight], None, [weight_scale], None, None, None, [qx_scale],
