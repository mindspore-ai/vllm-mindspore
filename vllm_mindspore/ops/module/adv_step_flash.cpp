...
uint8_t *GetDataPtr(const BaseTensorPtr &t) {
  return static_cast<uint8_t *>(t->device_address()->GetMutablePtr()) +
         t->data().itemsize() * t->storage_offset();
}

struct DtypeCaster {
  BaseTensorPtr CheckAndCast(const BaseTensorPtr &t,
                             const std::string &name = "") {
    mindspore::Int64ImmPtr dst_type = std::make_shared<mindspore::Int64Imm>(
        mindspore::TypeId::kNumberTypeInt32);
  ...
  BaseTensorPtr RecoveryTensorDtype(const BaseTensorPtr &t,
                                    const std::string &name) {
    ...
    auto ori_dtype =
        std::make_shared<mindspore::Int64Imm>(ori_tensor->data_type());
...
void AdvStepFlashAscendC(int32_t num_seqs, int32_t num_queries,
                         int32_t block_size,
  ...
  auto device_context =
      mindspore::runtime::OpRunner::GetDeviceContext("Ascend");
  PyBoostUtils::PrepareOpInputs(device_context, stream_id, input_tokens,
                                sampled_token_ids, input_positions, seq_lens,
                                slot_mapping, block_tables);
  // PyBoostUtils::PrepareOpOutputs(device_context, stream_id, outputs);
  PyBoostUtils::DispatchRun(
      std::make_shared<mindspore::runtime::PyBoostDeviceTask>([=]() {
        PyBoostUtils::MallocOpInputs(device_context, input_tokens,
                                     sampled_token_ids, input_positions,
        ...
        mindspore::runtime::OpExecutor::DispatchLaunchTask([=]() {
          ...
          void *l2ctrl = nullptr;
          AdvStepFlashKernelEntry(
              blockDims, l2ctrl, aclStream, sampledTokenIdsPtr, blockTablesPtr,
              seqLensPtr, inputTokensPtr, inputPositionsPtr, seqLensPtr,
              slotMappingPtr, num_seqs, block_size, block_tables_stride);
        });
      }));
...
MS_EXTENSION_MODULE(adv_step_flash) {
  m.def("adv_step_flash", &AdvStepFlashAscendC, "adv_step_flash_ascendc",
        pybind11::arg("num_seqs"), pybind11::arg("num_queries"),
        pybind11::arg("block_size"), pybind11::arg("input_tokens"),
        pybind11::arg("sampled_token_ids"), pybind11::arg("input_positions"),
        pybind11::arg("seq_lens"), pybind11::arg("slot_mapping"),
