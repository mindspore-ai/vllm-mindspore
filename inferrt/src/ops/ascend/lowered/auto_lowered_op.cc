...
AutoLoweredOp::AutoLoweredOp(const std::string &mlirText) : spec_(nullptr), executor_(nullptr) {
  ...
  RT_VLOG(VL_OPS) << "AutoLoweredOp created with MLIR text";
}
...
OpsErrorCode AutoLoweredOp::CalcWorkspace(const std::vector<const ir::Value *> &input, const ir::Value *output,
  ...
  RT_VLOG(VL_OPS) << "AutoLoweredOp::CalcWorkspace - calling executor";

  // Delegate to executor
  int ret = executor_->GetWorkspaceSize(workspaceSize, input, output);
  if (ret != 0) {
    RT_GLOG(ERROR) << "LoweredKernelExecutor::GetWorkspaceSize failed with code: " << ret;
    return UNKNOWN_ERROR;
  }

  RT_VLOG(VL_OPS) << "AutoLoweredOp::CalcWorkspace - workspace size: " << *workspaceSize;
  return SUCCESS;
}
...
OpsErrorCode AutoLoweredOp::Launch(const std::vector<const ir::Value *> &input, void *workspace, size_t workspaceSize,
  ...
  RT_VLOG(VL_OPS) << "AutoLoweredOp::Launch - calling executor";

  // Delegate to executor
  int ret = executor_->Launch(workspace, workspaceSize, stream, input, output);
  if (ret != 0) {
    RT_GLOG(ERROR) << "LoweredKernelExecutor::Launch failed with code: " << ret;
    return UNKNOWN_ERROR;
  }

  RT_VLOG(VL_OPS) << "AutoLoweredOp::Launch - success";
  return SUCCESS;
}
