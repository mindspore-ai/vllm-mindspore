...
inline void GetSlotStr(const Slot &slot, std::stringstream &ss) {
  switch (slot.type) {
    ...
    case SlotFloat: {
      ss << std::setprecision(std::numeric_limits<double>::max_digits10) << slot.value.float_;
      break;
    }
    case SlotString: {
      ss << slot.value.str_;
...
class StringPool {
 public:
  const char *Intern(const char *str) { return stringPool_.emplace(str).first->c_str(); }

  const char *Intern(const std::string &str) { return stringPool_.emplace(str).first->c_str(); }
  const char *Intern(const std::string &&str) { return stringPool_.emplace(std::move(str)).first->c_str(); }
...
#define BINARY_OP(OpName, OpSymbol)                                                                                  \
  void VM::InstBinary##OpName(ssize_t offset) {                                                                      \
    ...
    if (lhs.type == SlotInt && rhs.type == SlotInt) {                                                                \
      ...
      slot.type = SlotInt;                                                                                           \
      slot.value.int_ = res;                                                                                         \
      LOG_OUT << "result: " << ToString(lhs) << ' ' << #OpSymbol << ' ' << ToString(rhs) << " = " << ToString(slot); \
      ...
      if (strcmp(#OpSymbol, "+") != 0) {                                                                             \
        CompileMessage(LineString(), "error: only support '+' for string operation.");                               \
        exit(EXIT_FAILURE);                                                                                          \
      }                                                                                                              \
      std::stringstream ss;                                                                                          \
      GetSlotStr(lhs, ss);                                                                                           \
...
#define COMPARE_OP(CompareOpSymbol)                                                                                    \
  bool operator CompareOpSymbol(const Slot &lhs, const Slot &rhs) {                                                    \
    if (lhs.type == SlotInt && rhs.type == SlotInt) {                                                                  \
      ...
      LOG_OUT << "compare II result: " << (res ? "true" : "false") << ", " << ToString(lhs) << ' ' << #CompareOpSymbol \
              << ' ' << ToString(rhs);                                                                                 \
      return res;                                                                                                      \
    } else if (lhs.type == SlotFloat && rhs.type == SlotFloat) {                                                       \
      auto lhsVal = lhs.value.float_;                                                                                  \
      auto rhsVal = rhs.value.float_;                                                                                  \
      auto res = lhsVal CompareOpSymbol rhsVal;                                                                        \
      LOG_OUT << "compare FF result: " << (res ? "true" : "false") << ", " << ToString(lhs) << ' ' << #CompareOpSymbol \
              << ' ' << ToString(rhs);                                                                                 \
      return res;                                                                                                      \
    } else if (lhs.type == SlotInt && rhs.type == SlotFloat) {                                                         \
      auto lhsVal = lhs.value.int_;                                                                                    \
      ...
      LOG_OUT << "compare IF result: " << (res ? "true" : "false") << ", " << ToString(lhs) << ' ' << #CompareOpSymbol \
              << ' ' << ToString(rhs);                                                                                 \
      return res;                                                                                                      \
    } else if (lhs.type == SlotFloat && rhs.type == SlotInt) {                                                         \
      auto lhsVal = lhs.value.float_;                                                                                  \
      ...
      LOG_OUT << "compare FI result: " << (res ? "true" : "false") << ", " << ToString(lhs) << ' ' << #CompareOpSymbol \
              << ' ' << ToString(rhs);                                                                                 \
      return res;                                                                                                      \
    } else if (lhs.type == SlotString && rhs.type == SlotString) {                                                     \
      auto res = strcmp(lhs.value.str_, rhs.value.str_) CompareOpSymbol 0;                                             \
      LOG_OUT << "compare STR result: " << (res ? "true" : "false") << ", " << ToString(lhs) << ' '                    \
              << #CompareOpSymbol << ' ' << ToString(rhs);                                                             \
      return res;                                                                                                      \
    } else {                                                                                                           \
