...
# pylint: disable=redefined-builtin
def aten_op_func(input, memory_formats):
    """golden for aten.clone"""
    return torch.ops.aten.clone(input, memory_format=memory_formats)


# pylint: disable=redefined-builtin
def aten_op_func_default(input):
    """golden for aten.clone default"""
    return torch.ops.aten.clone(input)
...
def get_aten_op_func_compiled():
    """aten.clone op"""
    def custom_op_func(x, memory_formats):
        return torch.ops.aten.clone(x, memory_format=memory_formats)
    return torch.compile(custom_op_func, backend=backend)


def get_aten_op_func_compiled_default():
    """aten.clone op default"""
    def custom_op_func(x):
...
@pytest.mark.parametrize("shapes", [[9, 2], [32, 16, 4096], [2, 5, 9, 2]])
@pytest.mark.parametrize("dtypes", [torch.float16, torch.bfloat16, torch.float32])
def test_aten_clone_default(shapes, dtypes, ):
    """
    Feature: Test aclnn aten.clone default
    Description: Test aclnn aten.clone
    Expectation: The result is correct
    """
    cpu_input0 = torch.rand(shapes, dtype=dtypes)
    npu_input0 = cpu_input0.npu()

    cpu_output = aten_op_func_default(cpu_input0)
    op_func_compiled = get_aten_op_func_compiled_default()
    npu_output = op_func_compiled(npu_input0)
    npu_output_cpu = npu_output.cpu()
    AssertRtolEqual(cpu_output, npu_output_cpu)
...
@pytest.mark.parametrize("memory_formats", [torch.contiguous_format])
@pytest.mark.parametrize("dtypes", [torch.float16, torch.bfloat16, torch.float32])
def test_aten_clone_contiguous_format(shapes, memory_formats, dtypes, ):
    """
    Feature: Test aclnn aten.clone contiguous_format
    Description: Test aclnn aten.clone
    Expectation: The result is correct
    """
    cpu_input0 = torch.rand(shapes, dtype=dtypes)
    ...
    cpu_output = aten_op_func(cpu_input0, memory_formats)
    op_func_compiled = get_aten_op_func_compiled()
    npu_output = op_func_compiled(npu_input0, memory_formats)
    npu_output_cpu = npu_output.cpu()
    AssertRtolEqual(cpu_output, npu_output_cpu)
...
@pytest.mark.parametrize("memory_formats", [torch.preserve_format])
@pytest.mark.parametrize("dtypes", [torch.float16, torch.bfloat16, torch.float32])
def test_aten_clone_preserve_format(shapes, memory_formats, dtypes, ):
    """
    Feature: Test aclnn aten.clone preserve_format
    Description: Test aclnn aten.clone
    Expectation: The result is correct
    """
    cpu_input0 = torch.rand(shapes, dtype=dtypes)
...
@pytest.mark.parametrize("memory_formats", [torch.channels_last])
@pytest.mark.parametrize("dtypes", [torch.float16, torch.bfloat16, torch.float32])
def test_aten_clone_channels_last(shapes, memory_formats, dtypes, ):
    """
    Feature: Test aclnn aten.clone channels_last
    Description: Test aclnn aten.clone
    Expectation: The result is correct
    """
    cpu_input0 = torch.rand(shapes, dtype=dtypes)
...
@pytest.mark.parametrize("memory_formats", [torch.channels_last_3d])
@pytest.mark.parametrize("dtypes", [torch.float16, torch.bfloat16, torch.float32])
def test_aten_clone_channels_last_3d(shapes, memory_formats, dtypes, ):
    """
    Feature: Test aclnn aten.clone channels_last_3d
    Description: Test aclnn aten.clone
    Expectation: The result is correct
    """
    cpu_input0 = torch.rand(shapes, dtype=dtypes)
