...
"""Tests for torch.matmul operation."""
import numpy as np
import torch
...
def get_fp16_precision(d_range, k_range):
    """Get the precision for fp16"""
    prec16 = 1e-3
    for elm in PRECISION_TABLE_FP16:
        if elm[0] == d_range and elm[1] == k_range:
            return elm[2]
    return prec16


def assert_rtol_equal_matmul(x, y):
    """
    ...
    if x.dtype in (np.float16, np.float32):
        prec16 = get_fp16_precision(d_range, k_range)
