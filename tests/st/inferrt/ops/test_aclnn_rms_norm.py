...
def test_rms_norm(pipeline, monkeypatch):
    """
    ...
    npu_output0, npu_output1 = [npu_output.detach().cpu().numpy() for npu_output in op_func_compiled(npu_input0, npu_input1)]
...
@arg_mark(plat_marks=["platform_ascend"], level_mark="level0", card_mark="onecard", essential_mark="essential")
@pytest.mark.parametrize("pipeline", (True, False))
def test_rms_norm_mix_dtype(pipeline, monkeypatch):
    """
    Feature: Test aclnn rms_norm
