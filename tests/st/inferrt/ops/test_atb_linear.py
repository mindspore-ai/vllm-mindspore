"""Tests for ATB linear operation."""
import pytest
import torch
...
@pytest.mark.parametrize("in_features", [15, 25])
@pytest.mark.parametrize("out_features", [24, 88])
@pytest.mark.parametrize("dtype", (torch.float16, torch.bfloat16))
@pytest.mark.parametrize("has_bias", [False, True])
def test_atb_linear(dtype, batch_size, in_features, out_features, has_bias):
    """
    Feature: Test ATB linear
    Description: Test ATB linear with fp16/bf16 inputs
...
@pytest.mark.parametrize("has_bias", [False, True])
def test_atb_linear_3d(dtype, batch_size, in_features, out_features, has_bias):
    """
    Feature: Test ATB linear
    Description: Test ATB linear with 3D fp16/bf16 inputs
    Expectation: The result is correct
    """
