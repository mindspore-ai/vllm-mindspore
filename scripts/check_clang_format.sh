...
if [ "X${mode}" == "Xall" ]; then
  find {mopt,inferrt} -type f -name "*" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
elif [ "X${mode}" == "Xchanged" ]; then
  # --diff-filter=ACMRTUXB will ignore deleted files in commit
  git diff --diff-filter=ACMRTUXB --name-only | grep "mopt\|inferrt" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
else  # "X${mode}" == "Xlastcommit"
  git diff --diff-filter=ACMRTUXB --name-only HEAD~ HEAD | grep "mopt\|inferrt" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
fi
...
echo "${fail_flag}" > "${CHECK_RESULT_FILE}"
